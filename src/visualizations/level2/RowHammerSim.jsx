...
export default function RowHammerSim() {
  ...
  return (
    <div className="bg-dram-surface rounded-xl p-6 border border-dram-border">
      <h3 className="text-sm font-semibold text-dram-muted uppercase tracking-wider mb-4">
        RowHammer Simulator — Select aggressor row and hammer
      </h3>

      <div className="flex flex-col lg:flex-row gap-6">
        {/* Grid */}
        <div className="flex-shrink-0">
          <div className="text-xs text-dram-muted mb-2">Click a row to select as aggressor</div>
          ...
          <div className="mt-2 text-xs text-dram-muted">
            ⚔ = aggressor &nbsp;⚡ = victim &nbsp;
            <span className="text-red-400">red bits = flipped</span>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-4">
          <div>
            <div className="text-xs font-semibold text-dram-muted uppercase mb-2">Attack mode</div>
            <div className="flex gap-2">
              {[['single', 'Single-sided'], ['double', 'Double-sided']].map(([v, lbl]) => (
            ...
            <p className="text-xs text-dram-muted mt-1">
              Double-sided attacks two rows (aggressor & aggressor+2) simultaneously, distressing row aggressor+1 from both sides.
            </p>
          </div>
          ...
          {flipLog.length > 0 && (
            <div className="rounded-lg p-3 bg-red-900/10 border border-red-700/30">
              <div className="text-xs font-semibold text-red-400 mb-1">Bit flip log</div>
              <div className="text-xs font-mono text-red-300/70 space-y-0.5">
                {flipLog.slice(-8).map((f, i) => (
                  <div key={i}>{f}: flipped</div>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="rounded-lg p-3 bg-dram-bg text-xs text-dram-muted space-y-1">
            <div className="font-semibold text-dram-text">Rowhammer background</div>
            <div>Kim et al. (2014, ISCA) showed ~{(HAMMER_THRESHOLD/1000).toFixed(0)}k–150k activations within 64 ms tREFW can flip bits in victim rows. Mechanism: repeated charge pumping through the NMOS access transistor creates hot carriers that penetrate the gate oxide of adjacent rows, disturbing stored charge.</div>
          </div>
        </div>
      </div>
    </div>
