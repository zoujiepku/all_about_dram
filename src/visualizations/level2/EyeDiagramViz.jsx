...
export default function EyeDiagramViz() {
  ...
  return (
    <div className="bg-dram-surface rounded-xl p-6 border border-dram-border">
      <h3 className="text-sm font-semibold text-dram-muted uppercase tracking-wider mb-4">
        PAM4 Eye Diagram — Noise & Equalization
      </h3>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="flex-1 space-y-4">
          <div>
            <div className="text-xs font-semibold text-dram-muted uppercase mb-2">Modulation</div>
            <div className="flex gap-2">
              {[['nrz', 'NRZ (2-level)'], ['pam4', 'PAM4 (4-level)']].map(([v, lbl]) => (
          ...
          <div className="rounded-lg p-3 bg-dram-bg text-xs space-y-1">
            <div className="flex justify-between">
              <span className="text-dram-muted">Eye opening</span>
            ...
            {modulation === 'pam4' && (
              <div className="flex justify-between">
                <span className="text-dram-muted">PAM4 SNR penalty vs NRZ</span>
                <span className="font-mono text-amber-400">−{snrPenalty} dB</span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-dram-muted">Equalization active</span>
              <span className={`font-mono ${(ctleGain > 0 || dfeEnabled) ? 'text-dram-green' : 'text-dram-muted'}`}>
                {ctleGain > 0 ? `CTLE(${ctleGain}dB)` : ''}{ctleGain > 0 && dfeEnabled ? '+' : ''}{dfeEnabled ? 'DFE' : ''}{!ctleGain && !dfeEnabled ? 'none' : ''}
              </span>
            </div>
          </div>
