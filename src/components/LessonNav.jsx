...
export default function LessonNav({ lessonId, onComplete, lessons: propLessons }) {
  const ctx = useOutletContext() || {}
  const lang = ctx.lang || 'en'
  const lessons = propLessons ?? defaultLessons
  const idx = lessons.findIndex((l) => l.id === lessonId)
  const prev = idx > 0 ? lessons[idx - 1] : null
  const next = idx < lessons.length - 1 ? lessons[idx + 1] : null

  const labelPrev = lang === 'zh' ? '上一课' : 'Previous'
  const labelNext = lang === 'zh' ? '下一课' : 'Next'
  const labelMark = lang === 'zh' ? '标记完成 ✓' : 'Mark Complete ✓'
    ...
    <div className="mt-12 pt-6 border-t border-dram-border flex items-center justify-between gap-4">
      {prev ? (
        ...
        >
          ...
          <div>
            <div className="text-xs text-dram-muted/60">{labelPrev}</div>
      ...
      {next ? (
        ...
        >
          <div>
            <div className="text-xs text-dram-muted/60">{labelNext}</div>
