...
export default function L2_04() {
  ...
  return (
    <div className="lesson-prose">
      ...
      <p>
        优点：FR-FCFS 最大化行缓冲区局部性，大幅提升具有空间局部性的工作负载（如数据库扫描、GPU 纹理获取）的吞吐量。代价：若命中请求持续涌入，行缓冲区缺失请求可能被无限期推迟——即<strong>请求饥饿</strong>问题。
      </p>
      <p>
        现代控制器引入<strong>年龄阈值</strong>机制：等待超过 τ 个时钟周期的缺失请求将被提升为"高优先级"，无论其行缓冲区状态如何，从而避免无限延迟。
      </p>
      ...
      <div className="rounded-lg overflow-hidden border border-dram-border text-xs mt-4 mb-4">
        ...
        {[
          ['FR-FCFS + open page', '高空间局部性（数据库、GPU）', '随机访问、多请求方混合负载'],
          ['FCFS + closed page', '公平访问、实时性保障', '流式访问（浪费行缓冲区命中机会）'],
      ...
      <h2>延迟分布</h2>
      <p>
        在真实系统中，CPU 观测到的内存访问延迟并非固定值，而是一个分布。在使用开放页 FR-FCFS 策略且工作集较热的情况下，80–90% 的请求为命中（tCL ≈ 16 ns）。在指针追踪等随机访问工作负载下，90% 以上为缺失（≈48 ns）。尾部延迟（第 99 百分位）对延迟敏感型应用（如内存数据库和实时分析）至关重要——这也是硬件预取器和 QoS 感知调度器存在的动机。
      </p>
